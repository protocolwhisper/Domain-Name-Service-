...
contract Domains is ERC721URIStorage {
    ...
    constructor(string memory rug)
        payable
        ERC721("Rugged Domain Service", "RUG")
    {
        rugged = rug;
        console.log("%s name service deployed ", rug);
    }
    ...
    function price(string calldata name) public pure returns (uint256) {
        ...
        require(len > 0);
        if (len == 3) {
            return 5 * 10 * 16; //5 MATIC 18 decimals
        } else if (len == 4) {
            return 3 * 10 * 16; // 3 MATIC 18 DECIMALS
        } else {
            return 1 * 10 * 16;
        }
    }
    ...
    function register(string calldata name) public payable {
        ...
        string memory _name = string(abi.encodePacked(name, ".", rugged));
        ...
        // Let's create the json Metadata
        string memory json = Base64.encode(
            abi.encodePacked(
                '{"name": "',
                _name,
                '", "description": "A domain on the Rugged Name Service", "image": "data:image/svg+xml;base64,',
